...
**파일**: model_sample/moviemong/core.py

**개념**: 사용자가 영화를 보고 리뷰를 남기면, '리뷰몽'이 경험치(EXP)와 팝콘(재화)을 먹고 성장합니다.

**성장 단계**:
*   **Lv.1 (Egg)**: 알 상태
*   **Lv.2 ~ 5 (Toddler)**: 유아기 (부화)
*   **Lv.6 ~ 14 (Child)**: 아동기
*   **Lv.15+**: 청소년기 및 성체 (Adult)

### **3.3.2 밥주기 (Feeding) 시스템**

**파일**: model_sample/moviemong/feeding.py
...
*   🥤 **콤보 (B급, 15%)**: EXP +80
*   🦑 **오징어 (A급, 9%)**: EXP +150
*   🍗 **치킨 (S급, 1%)**: EXP +500 (대박 보상)

### **3.3.3 리뷰 분석 및 맛 스탯**

**파일**: model_sample/review.py

**기능**: 사용자가 쓴 리뷰 텍스트를 분석하여 8가지 '맛(Flavor)' 스탯을 올림.
...
| :--- | :--- | :--- |
| `id` | Integer | PK |
| `username` | String(80) | 사용자 계정명 |
| `level` | Integer | 캐릭터 레벨 (1~30) |
| `exp` | Integer | 현재 경험치 |
| `popcorn` | Integer | 보유 재화 |
| `stage` | String(20) | 성장 단계 (Egg, Toddler...) |
| `main_flavor` | String(50) | 대표 취향 (성격) |

### **3.4.2 취향 스탯 테이블 (`flavor_stats`)**

| 필드명 | 타입 | 설명 |
| :--- | :--- | :--- |
| `id` | Integer | PK |
| `user_id` | Integer | FK(`users.id`) |
| `flavor_name` | String(50) | 맛 이름 (Sweet...) |
| `score` | Integer | 누적 점수 |

### **3.4.3 질문 기록 테이블 (`question_history`)**

| 필드명 | 타입 | 설명 |
| :--- | :--- | :--- |
| `id` | Integer | PK |
| `user_id` | Integer | FK(`users.id`) |
| `date` | String(20) | 질문 날짜 |
| `question` | String(200) | 질문 내용 |
| `answer` | Text | 사용자 답변 |
...
model_sample/
├── analysis/
│   ├── sentiment.py           # [A-1] 감정/서사 분석 (LLM + Fallback)
│   ├── embedding.py           # [A-2] 영화 프로필 생성
│   ├── similarity.py          # [A-3] 만족 확률 계산 core
│   ├── description.py         # [A-5] 추천 설명 생성 (LLM)
│   ├── group_recommendation.py # [A-6] 그룹 취향 통합 (Least Misery)
│   ├── clustering.py          # [A-7] 장르 간 추천 (Round-Robin)
│   └── visualization.py       # [A-8] 워드클라우드 시각화
│
├── moviemong/
│   ├── core.py                # [Game] 캐릭터 성장/상태 관리
│   ├── feeding.py             # [Game] 밥주기 룰렛 시스템
│   └── review.py              # [Game] 리뷰 분석 및 맛 스탯
│
├── 유틸리티
│   ├── preference.py          # [Util] 선호도 추출 빌더
│   └── vector_utils.py        # [Util] 벡터 연산
│
├── 데이터
│   ├── data/moviemong.db      # [DB] SQLite (User, Feed, Flavor)
│   ├── emotion_tag.json       # [Data] 택소노미
│   └── .env                   # [Config] AWS 설정
│
└── 문서
